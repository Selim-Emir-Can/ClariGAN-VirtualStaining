...
def stratified_kfold_85_5_10(train_dir="train", k=10, seed=42):
    ...
    # Determine which letter+5x5 classes are too small
    for item in data:
        ...
        if count < k:
            if letter == "P":
                ...
                # Merge small 5x5 class with its 10x10 counterpart
                counterpart_label = f"{letter}_10x10"
                if label_counts.get(counterpart_label, 0) >= k:
                    item["crop"] = "10x10"
                else:
                    # fallback: just merge into original 5x5 (may fail later)
                    pass
...
#         if os.path.exists(r3_path):
#             data.append((r1_path, r3_path))
#             labels.append(label)

#     skf = StratifiedKFold(n_splits=k, shuffle=True, random_state=seed)
#     all_indices = list(skf.split(data, labels))
...
#         # From trainval (~90%), stratify 5% for validation
#         val_ratio = 1 / 18  # 5% of total ≈ 1/18 of the remaining 90%
#         train_data, val_data, _, _ = train_test_split(
#             trainval_data, trainval_labels,
#             test_size=val_ratio,
#             stratify=trainval_labels,
...
def stratified_split_by_letter_and_crop(train_dir="train", k=5, seed=42):
    input_dir = os.path.join(train_dir, 'A')
    gt_dir = os.path.join(train_dir, 'B')
    ...
    # Pattern to extract letter and crop size (e.g., R1-A_5x5_img001.png → label = A_5x5)
    pattern = re.compile(r'R1-([A-Za-z]+(?:part\d+)?).*?_(5x5|10x10)')
...
def get_labels_from_split(split):
    ...
    for r1_path, _ in split:
        ...
        if match:
            letter = match.group(1)
            crop = match.group(2)
            labels.append(f"{letter}_{crop}")
    return labels


def get_labels_from_split_BFDF(split):
    labels = []
    # Match 'SkinB1', 'B1', 'SkinA2', etc., at the start of the filename
    pattern = re.compile(r'^(Skin[A-Z]\d|[A-Z]\d)')
    for r1_path, _ in split:
        filename = os.path.basename(r1_path)
...
if __name__ == "__main__":
    splits = stratified_kfold_85_5_10(r"C:\Users\ammic\Desktop\BBDM-kfold\train", k=10) # change the path to BBDM dataset train directory
    ...
    CONFIG_PATH = r"C:\Users\ammic\Desktop\ClariGAN-DL\BBDM\configs\Template-BBDM_pixel_256_matchUNet.yaml"
    MODEL_DIR_NAME = "BBDM-pixel-matchUNet"   # part of the on-disk results path
    SUFFIX = "pixel_BBDM_matchUNet"           # used in save_name / sample_path

    sys.argv = [
        "k-fold_validation_pixel_BBDM.py",
        "--config", CONFIG_PATH,
        "--train",
        "--sample_at_start",
        "--save_top",
    ...
    for fold in splits:
        ...
        save_name = f'fold_{counter}_{SUFFIX}'
        ...
        nconfig.data.dataset_config.dataset_path = r"C:\Users\ammic\Desktop\BBDM-kfold" # change the path to BBDM dataset train directory
        ...
        def find_latest_ckpt(base_path, dataset_name):
            ckpt_dir = os.path.join(base_path, dataset_name, MODEL_DIR_NAME, "checkpoint")
        ...
        sample_path = os.path.join(rf"C:\Users\ammic\Desktop\ClariGAN-DL\k-fold_results_{SUFFIX}", save_name) # WHERE YOU HAVE TEST RESULTS FOR EACH FOLD of the k-fold cross validation
