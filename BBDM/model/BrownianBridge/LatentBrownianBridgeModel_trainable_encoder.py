...
class LatentBrownianBridgeModel(BrownianBridgeModel):
    def __init__(self, model_config):
        ...
        # Freeze everything in VQGAN except the encoder
        self.vqgan.train = disabled_train
        for param in self.vqgan.parameters():
            param.requires_grad = False

        # Unfreeze encoder and quant_conv so gradients flow through encoding
        for param in self.vqgan.encoder.parameters():
            param.requires_grad = True
        for param in self.vqgan.quant_conv.parameters():
            param.requires_grad = True
        self.vqgan.encoder.train()

        print(f"load vqgan from {model_config.VQGAN.params.ckpt_path}")

        encoder_trainable = all(p.requires_grad for p in self.vqgan.encoder.parameters())
        quant_conv_trainable = all(p.requires_grad for p in self.vqgan.quant_conv.parameters())
        decoder_frozen = not any(p.requires_grad for p in self.vqgan.decoder.parameters())
        print(f"[DEBUG] vqgan.encoder trainable:   {encoder_trainable}")
        print(f"[DEBUG] vqgan.quant_conv trainable: {quant_conv_trainable}")
        print(f"[DEBUG] vqgan.decoder frozen:       {decoder_frozen}")
    ...
    def get_parameters(self):
        if self.condition_key == 'SpatialRescaler':
            print("get parameters to optimize: SpatialRescaler, UNet, encoder")
            ...
            print("get parameters to optimize: UNet, encoder")
            params = itertools.chain(
                self.denoise_fn.parameters(),
                self.vqgan.encoder.parameters(),
                self.vqgan.quant_conv.parameters(),
    ...
    def forward(self, x, x_cond, context=None):
        ...
        return super().forward(x_latent, x_cond_latent, context)
