...
if __name__ == "__main__":
    ...
    nconfig.data.test.batch_size = 8
    ...
    val_results_dir = r"C:\Users\ammic\Desktop\ClariGAN-DL\BBDM\AdapterNetwork_Weights\validation_results"
    ...
    adapter_ckpt = r"C:\Users\ammic\Desktop\ClariGAN-DL\BBDM\AdapterNetwork_Weights\adapter_model_best_e1.pth"
    if(adapter_ckpt != ""):
        epoch_start = int(adapter_ckpt.split("_e")[-1].split(".pth")[0])
    else:
        epoch_start = 0
    adapter.load_state_dict(torch.load(adapter_ckpt, weights_only=True, map_location='cpu'))
    ...
    for epoch in range(epoch_start,num_epochs):
        ...
        for batch_idx, train_batch in enumerate(pbar):
            ...
            try:
                ...
                if (batch_idx + 1) % 100 == 0:  # Save more frequently (reduced from 100)
        ...
        if batch_count > 0:
            ...
            val_pbar = tqdm(val_loader, smoothing=0.01) if val_loader else None
                ...
                with torch.no_grad():  # No gradients needed for validation
                    for val_batch_idx, val_batch in enumerate(val_pbar):
                        try:
                            ...
                            (val_x, _), (val_x_cond, _) = val_batch
                            del val_batch
                            
                            # Move conditional input to GPU
                            val_x_cond = val_x_cond.to(device)
                            val_x = val_x.to('cpu')  # Keep ground truth on CPU
                            
                            # Process with BBDM model
                            bbdmnet.to(device)
                            val_result = bbdmnet.sample(val_x_cond, clip_denoised=runner.config.testing.clip_denoised).detach()
                            val_adapter_input = val_result.clone()
                            ...
                            val_adapter_output = adapter(val_adapter_input.unsqueeze(0), val_x_cond)
                            
                            # Compute validation loss
                            val_loss = criterion(val_adapter_output, val_x)
                            val_current_loss = val_loss.item()
                            if((val_batch_idx < 3) or (val_batch_idx % 10 == 0)):
                                with torch.no_grad():
                                    pred_grid = get_image_grid(val_adapter_output.cpu(), grid_size=1, to_normal=True)
                                    label_grid = get_image_grid(val_x.cpu(), grid_size=1, to_normal=True)
                            ...
                            val_total_loss += val_current_loss
                            val_batch_count += 1
                            
                            # Free memory
                            del val_adapter_input, val_x_cond, val_adapter_output, val_x, val_loss
                            torch.cuda.empty_cache()
                            gc.collect()
                            
                            # Update progress bar
                            if device.type == 'cuda':
                                val_mem_allocated = torch.cuda.memory_allocated() / 1e9
                                val_mem_reserved = torch.cuda.memory_reserved() / 1e9
                                val_pbar.set_description(f"Val Batch {val_batch_idx+1}, Loss: {val_current_loss:.4f}, GPU mem: {val_mem_allocated:.2f}/{val_mem_reserved:.2f} GB")
                            else:
                                val_pbar.set_description(f"Val Batch {val_batch_idx+1}, Loss: {val_current_loss:.4f}")
                                
                        except RuntimeError as e:
                            if "CUDA out of memory" in str(e):
                                print(f"\nCUDA OOM in validation batch {val_batch_idx}. Clearing memory and skipping batch.")
                                torch.cuda.empty_cache()
                                gc.collect()
                            else:
                                print(f"\nError in validation batch {val_batch_idx}: {e}")
                            # Skip to next batch
                            continue
                
                # Validation reporting
                if val_batch_count > 0:
                    val_avg_loss = val_total_loss / val_batch_count
                    print(f"Validation completed with average loss: {val_avg_loss:.4f}")
                    ...
                    if val_avg_loss < best_val_loss:
                        ...
                        best_model_path = os.path.join(save_dir, f"adapter_model_best_e{epoch}.pth")
                        torch.save(adapter.state_dict(), best_model_path)
                        print(f"New best model saved with validation loss: {val_avg_loss:.4f}")
