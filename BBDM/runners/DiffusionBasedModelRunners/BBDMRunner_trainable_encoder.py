import os

import torch.optim.lr_scheduler
from torch.utils.data import DataLoader

from PIL import Image
from Register import Registers
from model.BrownianBridge.BrownianBridgeModel import BrownianBridgeModel
from model.BrownianBridge.LatentBrownianBridgeModel_trainable_encoder import LatentBrownianBridgeModel
from runners.DiffusionBasedModelRunners.DiffusionBaseRunner import DiffusionBaseRunner
from runners.utils import weights_init, get_optimizer, get_dataset, make_dir, get_image_grid, save_single_image
from tqdm.autonotebook import tqdm
from torchsummary import summary

import matplotlib.pyplot as plt

from torchmetrics.image.ssim import StructuralSimilarityIndexMeasure

@Registers.runners.register_with_name('BBDMRunner_trainable_encoder')
class BBDMRunner(DiffusionBaseRunner):
    def __init__(self, config):
        super().__init__(config)

        if (hasattr(config, "label_by_name") and hasattr(config, "class_weights")):
            self.label_by_name = config.label_by_name
            self.class_weights = config.class_weights
        else:
            self.label_by_name = None
            self.class_weights = None


        self.ssim_fn = StructuralSimilarityIndexMeasure(data_range=1.0).to(self.config.training.device[0])

    def initialize_model(self, config):
        if config.model.model_type == "BBDM":
            bbdmnet = BrownianBridgeModel(config.model).to(config.training.device[0])
        elif config.model.model_type == "LBBDM":
            bbdmnet = LatentBrownianBridgeModel(config.model).to(config.training.device[0])
        else:
            raise NotImplementedError
        bbdmnet.apply(weights_init)
        return bbdmnet

    def load_model_from_checkpoint(self):
        states = None
        if self.config.model.only_load_latent_mean_std:
            if self.config.model.__contains__('model_load_path') and self.config.model.model_load_path is not None:
                states = torch.load(self.config.model.model_load_path, map_location='cpu')
        else:
            states = super().load_model_from_checkpoint()

        if self.config.model.normalize_latent:
            if states is not None:
                self.net.ori_latent_mean = states['ori_latent_mean'].to(self.config.training.device[0])
                self.net.ori_latent_std = states['ori_latent_std'].to(self.config.training.device[0])
                self.net.cond_latent_mean = states['cond_latent_mean'].to(self.config.training.device[0])
                self.net.cond_latent_std = states['cond_latent_std'].to(self.config.training.device[0])
            else:
                if self.config.args.train:
                    self.get_latent_mean_std()

    def print_model_summary(self, net):
        def get_parameter_number(model):
            total_num = sum(p.numel() for p in model.parameters())
            trainable_num = sum(p.numel() for p in model.parameters() if p.requires_grad)
            return total_num, trainable_num

        total_num, trainable_num = get_parameter_number(net)
        self.logger("Total Number of parameter: %.2fM" % (total_num / 1e6))
        self.logger("Trainable Number of parameter: %.2fM" % (trainable_num / 1e6))

    def initialize_optimizer_scheduler(self, net, config):
        optimizer = get_optimizer(config.model.BB.optimizer, net.get_parameters())
        scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer=optimizer,
                                                               mode='min',
                                                               verbose=True,
                                                               threshold_mode='rel',
                                                               **vars(config.model.BB.lr_scheduler)
)
        return [optimizer], [scheduler]

    @torch.no_grad()
    def get_checkpoint_states(self, stage='epoch_end'):
        model_states, optimizer_scheduler_states = super().get_checkpoint_states()
        if self.config.model.normalize_latent:
            if self.config.training.use_DDP:
                model_states['ori_latent_mean'] = self.net.module.ori_latent_mean
                model_states['ori_latent_std'] = self.net.module.ori_latent_std
                model_states['cond_latent_mean'] = self.net.module.cond_latent_mean
                model_states['cond_latent_std'] = self.net.module.cond_latent_std
            else:
                model_states['ori_latent_mean'] = self.net.ori_latent_mean
                model_states['ori_latent_std'] = self.net.ori_latent_std
                model_states['cond_latent_mean'] = self.net.cond_latent_mean
                model_states['cond_latent_std'] = self.net.cond_latent_std
        return model_states, optimizer_scheduler_states

    def get_latent_mean_std(self):
        train_dataset, val_dataset, test_dataset = get_dataset(self.config.data)
        train_loader = DataLoader(train_dataset,
                                  batch_size=self.config.data.train.batch_size,
                                  shuffle=True,
                                  num_workers=8,
                                  drop_last=True)

        total_ori_mean = None
        total_ori_var = None
        total_cond_mean = None
        total_cond_var = None
        max_batch_num = 30000 // self.config.data.train.batch_size

        def calc_mean(batch, total_ori_mean=None, total_cond_mean=None):
            (x, x_name), (x_cond, x_cond_name) = batch
            x = x.to(self.config.training.device[0])
            x_cond = x_cond.to(self.config.training.device[0])

            x_latent = self.net.encode(x, cond=False, normalize=False)
            x_cond_latent = self.net.encode(x_cond, cond=True, normalize=False)
            x_mean = x_latent.mean(axis=[0, 2, 3], keepdim=True)
            total_ori_mean = x_mean if total_ori_mean is None else x_mean + total_ori_mean

            x_cond_mean = x_cond_latent.mean(axis=[0, 2, 3], keepdim=True)
            total_cond_mean = x_cond_mean if total_cond_mean is None else x_cond_mean + total_cond_mean
            return total_ori_mean, total_cond_mean

        def calc_var(batch, ori_latent_mean=None, cond_latent_mean=None, total_ori_var=None, total_cond_var=None):
            (x, x_name), (x_cond, x_cond_name) = batch
            x = x.to(self.config.training.device[0])
            x_cond = x_cond.to(self.config.training.device[0])

            x_latent = self.net.encode(x, cond=False, normalize=False)
            x_cond_latent = self.net.encode(x_cond, cond=True, normalize=False)
            x_var = ((x_latent - ori_latent_mean) ** 2).mean(axis=[0, 2, 3], keepdim=True)
            total_ori_var = x_var if total_ori_var is None else x_var + total_ori_var

            x_cond_var = ((x_cond_latent - cond_latent_mean) ** 2).mean(axis=[0, 2, 3], keepdim=True)
            total_cond_var = x_cond_var if total_cond_var is None else x_cond_var + total_cond_var
            return total_ori_var, total_cond_var

        self.logger(f"start calculating latent mean")
        batch_count = 0
        for train_batch in tqdm(train_loader, total=len(train_loader), smoothing=0.01):
            # if batch_count >= max_batch_num:
            #     break
            batch_count += 1
            total_ori_mean, total_cond_mean = calc_mean(train_batch, total_ori_mean, total_cond_mean)

        ori_latent_mean = total_ori_mean / batch_count
        self.net.ori_latent_mean = ori_latent_mean

        cond_latent_mean = total_cond_mean / batch_count
        self.net.cond_latent_mean = cond_latent_mean

        self.logger(f"start calculating latent std")
        batch_count = 0
        for train_batch in tqdm(train_loader, total=len(train_loader), smoothing=0.01):
            # if batch_count >= max_batch_num:
            #     break
            batch_count += 1
            total_ori_var, total_cond_var = calc_var(train_batch,
                                                     ori_latent_mean=ori_latent_mean,
                                                     cond_latent_mean=cond_latent_mean,
                                                     total_ori_var=total_ori_var,
                                                     total_cond_var=total_cond_var)
            # break

        ori_latent_var = total_ori_var / batch_count
        cond_latent_var = total_cond_var / batch_count

        self.net.ori_latent_std = torch.sqrt(ori_latent_var)
        self.net.cond_latent_std = torch.sqrt(cond_latent_var)
        self.logger(self.net.ori_latent_mean)
        self.logger(self.net.ori_latent_std)
        self.logger(self.net.cond_latent_mean)
        self.logger(self.net.cond_latent_std)

    def loss_fn(self, net, batch, epoch, step, opt_idx=0, stage='train', write=True):
        (x, x_name), (x_cond, x_cond_name) = batch
        x = x.to(self.config.training.device[0])
        x_cond = x_cond.to(self.config.training.device[0])

        """
        print('\n', x.shape, x_cond.shape, x.dtype, '\n')

        # Convert tensors to numpy for plotting
        x_np = x.permute(0, 2, 3, 1).cpu().numpy()
        x_cond_np = x_cond.permute(0, 2, 3, 1).cpu().numpy()
        x_np = x_np / 2.0 + 0.5
        x_cond_np = x_cond_np / 2.0 + 0.5

        # Extract just the filename without folder paths
        # This assumes x_name and x_cond_name are lists of paths
        x_basenames = [os.path.basename(name) for name in x_name]
        x_cond_basenames = [os.path.basename(name) for name in x_cond_name]

        # Create figure with 2x8 grid
        fig, axes = plt.subplots(2, 8, figsize=(16, 4))

        # Plot images with filenames as titles
        for i in range(8):
            axes[0, i].imshow(x_np[i])
            axes[0, i].set_title(x_basenames[i], fontsize=8)
            axes[0, i].axis('off')
            
            axes[1, i].imshow(x_cond_np[i])
            axes[1, i].set_title(x_cond_basenames[i], fontsize=8)
            axes[1, i].axis('off')

        plt.tight_layout()
        plt.show()
        print(x_name, x_cond_name)
        # raise Exception
        """
        
        

        loss, additional_info = net(x, x_cond)
        """
        print(additional_info["x0"].shape, net.decode(additional_info["x0"]).shape)
        # Convert tensors to numpy for plotting
        x_np = net.decode(additional_info["x0"]).permute(0, 2, 3, 1).cpu().numpy()
        x_cond_np = net.decode(additional_info["x0_recon"]).permute(0, 2, 3, 1).cpu().numpy()
        x_np = x_np / 2.0 + 0.5
        x_cond_np = x_cond_np / 2.0 + 0.5

        # Create figure with 2x8 grid
        fig, axes = plt.subplots(2, 8, figsize=(16, 4))

        # Plot images with filenames as titles
        for i in range(8):
            axes[0, i].imshow(x_np[i])
            axes[0, i].set_title("x0", fontsize=8)
            axes[0, i].axis('off')
            
            axes[1, i].imshow(x_cond_np[i])
            axes[1, i].set_title("x0_recon", fontsize=8)
            axes[1, i].axis('off')

        plt.tight_layout()
        plt.show()
        raise Exception
        """
        
        if ((stage == 'train') and (self.label_by_name is not None) and (self.class_weights is not None)):
            per_sample = additional_info['per_sample_loss']  # expected shape [B]

            if ((per_sample is not None) and (per_sample.ndim == 1) and (per_sample.shape[0] == x.shape[0])):
                # Start with all-ones; use dtype/device from per_sample
                weights = per_sample.new_full((per_sample.shape[0],), 1.0)

                # Map each filename -> label -> class weight
                for i, name in enumerate(x_name):
                    lbl = self.label_by_name.get(os.path.basename(name).split("_")[0])
                    if lbl is not None:
                        weights[i] = float(self.class_weights.get(lbl, 1.0))
                
                # Normalize to keep loss scale stable
                weights = weights / weights.mean().clamp_min(1e-8)

                # Optionally guard against NaN/Inf in both vectors
                mask = torch.isfinite(per_sample) & torch.isfinite(weights)
                if mask.any():
                    loss = (per_sample[mask] * weights[mask]).mean()

 

        if write and self.is_main_process:
            self.writer.add_scalar(f'loss/{stage}', loss, step)
            if additional_info.__contains__('recloss_noise'):
                self.writer.add_scalar(f'recloss_noise/{stage}', additional_info['recloss_noise'], step)
            if additional_info.__contains__('recloss_xy'):
                self.writer.add_scalar(f'recloss_xy/{stage}', additional_info['recloss_xy'], step)
        return loss

    @torch.no_grad()
    def sample(self, net, batch, sample_path, stage='train', multimodal=False):
        sample_path = make_dir(os.path.join(sample_path, f'{stage}_sample'))
        reverse_sample_path = make_dir(os.path.join(sample_path, 'reverse_sample'))
        reverse_one_step_path = make_dir(os.path.join(sample_path, 'reverse_one_step_samples'))

        print(sample_path)

        (x, x_name), (x_cond, x_cond_name) = batch

        batch_size = x.shape[0] if x.shape[0] < 4 else 4

        x = x[0:batch_size].to(self.config.training.device[0])
        x_cond = x_cond[0:batch_size].to(self.config.training.device[0])

        grid_size = 4

        # samples, one_step_samples = net.sample(x_cond,
        #                                        clip_denoised=self.config.testing.clip_denoised,
        #                                        sample_mid_step=True)
        # self.save_images(samples, reverse_sample_path, grid_size, save_interval=200,
        #                  writer_tag=f'{stage}_sample' if stage != 'test' else None)
        #
        # self.save_images(one_step_samples, reverse_one_step_path, grid_size, save_interval=200,
        #                  writer_tag=f'{stage}_one_step_sample' if stage != 'test' else None)
        #
        # sample = samples[-1]
        sample = net.sample(x_cond, clip_denoised=self.config.testing.clip_denoised).to('cpu')
        image_grid = get_image_grid(sample, grid_size, to_normal=self.config.data.dataset_config.to_normal)
        im = Image.fromarray(image_grid)
        im.save(os.path.join(sample_path, 'skip_sample.png'))
        if stage != 'test':
            self.writer.add_image(f'{stage}_skip_sample', image_grid, self.global_step, dataformats='HWC')
        
        if(multimodal == True): # multi-modal
            x_cond = x_cond[:,1,:,:,:]

        image_grid = get_image_grid(x_cond.to('cpu'), grid_size, to_normal=self.config.data.dataset_config.to_normal)
        im = Image.fromarray(image_grid)
        im.save(os.path.join(sample_path, 'condition.png'))
        if stage != 'test':
            self.writer.add_image(f'{stage}_condition', image_grid, self.global_step, dataformats='HWC')

        if(multimodal == True): # multi-modal
            x = x[:,1,:,:,:]

        image_grid = get_image_grid(x.to('cpu'), grid_size, to_normal=self.config.data.dataset_config.to_normal)
        im = Image.fromarray(image_grid)
        im.save(os.path.join(sample_path, 'ground_truth.png'))
        if stage != 'test':
            self.writer.add_image(f'{stage}_ground_truth', image_grid, self.global_step, dataformats='HWC')

    @torch.no_grad()
    def sample_to_eval(self, net, test_loader, sample_path):
        condition_path = make_dir(os.path.join(sample_path, f'condition'))
        gt_path = make_dir(os.path.join(sample_path, 'ground_truth'))
        result_path = make_dir(os.path.join(sample_path, str(self.config.model.BB.params.sample_step)))

        pbar = tqdm(test_loader, total=len(test_loader), smoothing=0.01)
        batch_size = self.config.data.test.batch_size
        to_normal = self.config.data.dataset_config.to_normal
        sample_num = self.config.testing.sample_num
        for test_batch in pbar:
            (x, x_name), (x_cond, x_cond_name) = test_batch
            x = x.to(self.config.training.device[0])
            x_cond = x_cond.to(self.config.training.device[0])

            for j in range(sample_num):
                sample = net.sample(x_cond, clip_denoised=False)
                # sample = net.sample_vqgan(x)
                for i in range(batch_size):
                    condition = x_cond[i].detach().clone()
                    gt = x[i]
                    result = sample[i]
                    if j == 0:
                        save_single_image(condition, condition_path, f'{x_cond_name[i]}.png', to_normal=to_normal)
                        save_single_image(gt, gt_path, f'{x_name[i]}.png', to_normal=to_normal)
                    if sample_num > 1:
                        result_path_i = make_dir(os.path.join(result_path, x_name[i]))
                        save_single_image(result, result_path_i, f'output_{j}.png', to_normal=to_normal)
                    else:
                        save_single_image(result, result_path, f'{x_name[i]}.png', to_normal=to_normal)

    @torch.no_grad()
    def sample_to_eval_combined(self, net, test_loader, sample_path):
        result_path = make_dir(os.path.join(sample_path, str(self.config.model.BB.params.sample_step)))

        pbar = tqdm(test_loader, total=len(test_loader), smoothing=0.01)
        batch_size = self.config.data.test.batch_size
        to_normal = self.config.data.dataset_config.to_normal
        sample_num = self.config.testing.sample_num

        from PIL import ImageDraw, ImageFont

        def add_titles_and_concat(condition, gt, result):
            """Adds titles to images and concatenates them side by side."""
            def add_title(img, title):
                draw = ImageDraw.Draw(img)
                font = ImageFont.load_default()  # Use the default PIL font
                text_bbox = font.getbbox(title)  # Use getbbox() instead of getsize()
                text_height = text_bbox[3] - text_bbox[1]  # Calculate text height
                draw.rectangle([(0, 0), (img.width, text_height + 5)], fill=(0, 0, 0))
                draw.text((5, 0), title, fill=(255, 255, 255), font=font)
                return img

            condition = add_title(condition, "Condition")
            gt = add_title(gt, "Ground Truth")
            result = add_title(result, "Output")

            total_width = condition.width + gt.width + result.width
            max_height = max(condition.height, gt.height, result.height)

            combined = Image.new('RGB', (total_width, max_height))
            combined.paste(condition, (0, 0))
            combined.paste(gt, (condition.width, 0))
            combined.paste(result, (condition.width + gt.width, 0))

            return combined


        for test_batch in pbar:
            (x, x_name), (x_cond, x_cond_name) = test_batch
            x = x.to(self.config.training.device[0])
            x_cond = x_cond.to(self.config.training.device[0])

            for j in range(sample_num):
                sample = net.sample(x_cond, clip_denoised=False)
                for i in range(batch_size):
                    condition = x_cond[i].detach().clone()
                    gt = x[i].detach().clone()  # Ensure ground truth is reset for each sample_num iteration
                    result = sample[i]

                    # Convert tensors to images
                    condition_img = condition.mul_(0.5).add_(0.5).clamp_(0, 1.).mul_(255).add_(0.5).clamp_(0, 255)
                    gt_img = gt.mul_(0.5).add_(0.5).clamp_(0, 1.).mul_(255).add_(0.5).clamp_(0, 255)
                    result_img = result.mul_(0.5).add_(0.5).clamp_(0, 1.).mul_(255).add_(0.5).clamp_(0, 255)

                    condition_img = condition_img.permute(1, 2, 0).to('cpu', torch.uint8).numpy()
                    gt_img = gt_img.permute(1, 2, 0).to('cpu', torch.uint8).numpy()
                    result_img = result_img.permute(1, 2, 0).to('cpu', torch.uint8).numpy()

                    condition_pil = Image.fromarray(condition_img)
                    gt_pil = Image.fromarray(gt_img)
                    result_pil = Image.fromarray(result_img)

                    combined_img = add_titles_and_concat(condition_pil, gt_pil, result_pil)

                    if sample_num > 1:
                        result_path_i = make_dir(os.path.join(result_path, x_name[i]))
                        save_name = f'output_{j}.png'
                        combined_img.save(os.path.join(result_path_i, save_name))
                    else:
                        save_name = f'{x_name[i]}.png'
                        combined_img.save(os.path.join(result_path, save_name))
    
    def sample_to_eval_combined_with_uncertainty(self, net, test_loader, sample_path, multimodal=False):
        from PIL import ImageDraw, ImageFont
        import numpy as np
        import matplotlib.pyplot as plt

        condition_path = make_dir(os.path.join(sample_path, 'condition'))
        gt_path = make_dir(os.path.join(sample_path, 'ground_truth'))
        result_path = make_dir(os.path.join(sample_path, str(self.config.model.BB.params.sample_step)))
        titled_plot_path = make_dir(os.path.join(sample_path, 'titled_plots'))
        uncertainty_path = make_dir(os.path.join(sample_path, 'uncertainty_maps'))

        pbar = tqdm(test_loader, total=len(test_loader), smoothing=0.01)
        batch_size = self.config.data.test.batch_size
        sample_num = self.config.testing.sample_num

        def add_titles_and_concat(condition, gt, result):
            """Adds titles to condition, ground truth, and output, concatenates them."""
            def add_title(img, title):
                draw = ImageDraw.Draw(img)
                font = ImageFont.load_default()
                text_bbox = font.getbbox(title)
                text_height = text_bbox[3] - text_bbox[1]
                draw.rectangle([(0, 0), (img.width, text_height + 5)], fill=(0, 0, 0))
                draw.text((5, 0), title, fill=(255, 255, 255), font=font)
                return img

            condition = add_title(condition, "Condition")
            gt = add_title(gt, "Ground Truth")
            result = add_title(result, "Output")

            total_width = condition.width + gt.width + result.width
            max_height = max(condition.height, gt.height, result.height)

            combined = Image.new('RGB', (total_width, max_height))
            combined.paste(condition, (0, 0))
            combined.paste(gt, (condition.width, 0))
            combined.paste(result, (condition.width + gt.width, 0))

            return combined

        for test_batch in pbar:
            (x, x_name), (x_cond, x_cond_name) = test_batch
            x = x.to(self.config.training.device[0])
            x_cond = x_cond.to(self.config.training.device[0])

            

            for i in range(batch_size):
                outputs = []
                for j in range(sample_num):
                    # print(x_cond[i].unsqueeze(0).shape)
                    sample = net.sample(x_cond[i].unsqueeze(0), clip_denoised=False)
                    result = sample[0]

                    

                    if j == 0:
                        if(multimodal == True):
                            save_single_image(x_cond[i][1,:,:,:], condition_path, f'{x_cond_name[i]}.png', to_normal=self.config.data.dataset_config.to_normal)
                            save_single_image(x[i][1,:,:,:], gt_path, f'{x_name[i]}.png', to_normal=self.config.data.dataset_config.to_normal)
                        else:
                            save_single_image(x_cond[i], condition_path, f'{x_cond_name[i]}.png', to_normal=self.config.data.dataset_config.to_normal)
                            save_single_image(x[i], gt_path, f'{x_name[i]}.png', to_normal=self.config.data.dataset_config.to_normal)

                    result_path_i = make_dir(os.path.join(result_path, x_name[i]))
                    save_single_image(result, result_path_i, f'output_{j}.png', to_normal=self.config.data.dataset_config.to_normal)
                    
                    
                    if(multimodal == True):
                        # Save titled plots without uncertainty
                        condition_img = (x_cond[i][1,:,:,:] * 0.5 + 0.5).clamp(0, 1).mul(255).permute(1, 2, 0).cpu().numpy().astype(np.uint8)
                        gt_img = (x[i][1,:,:,:] * 0.5 + 0.5).clamp(0, 1).mul(255).permute(1, 2, 0).cpu().numpy().astype(np.uint8)
                        result_img = (result * 0.5 + 0.5).clamp(0, 1).mul(255).permute(1, 2, 0).cpu().numpy().astype(np.uint8)
                    else:
                        # Save titled plots without uncertainty
                        condition_img = (x_cond[i] * 0.5 + 0.5).clamp(0, 1).mul(255).permute(1, 2, 0).cpu().numpy().astype(np.uint8)
                        gt_img = (x[i] * 0.5 + 0.5).clamp(0, 1).mul(255).permute(1, 2, 0).cpu().numpy().astype(np.uint8)
                        result_img = (result * 0.5 + 0.5).clamp(0, 1).mul(255).permute(1, 2, 0).cpu().numpy().astype(np.uint8)

                    combined_img = add_titles_and_concat(
                        Image.fromarray(condition_img),
                        Image.fromarray(gt_img),
                        Image.fromarray(result_img)
                    )
                    titled_plot_path_i = make_dir(os.path.join(titled_plot_path, x_name[i]))
                    combined_img.save(os.path.join(titled_plot_path_i, f'titled_plot_{j}_{x_name[i]}.png'))

                    outputs.append(result.detach())

                if len(outputs) > 1:
                    def rgb_to_grayscale_batch(rgb_tensor):
                        """
                        Convert a batch of RGB tensors to grayscale.
                        
                        Args:
                            rgb_tensor (torch.Tensor): Tensor of shape (N, C, H, W), where C = 3 for RGB.
                            
                        Returns:
                            torch.Tensor: Grayscale tensor of shape (N, 1, H, W).
                        """
                        if rgb_tensor.size(1) != 3:
                            raise ValueError("Input tensor must have 3 channels in the second dimension (N, C, H, W).")

                        # Apply the grayscale conversion formula along the channel dimension
                        grayscale_tensor = (
                            0.2989 * rgb_tensor[:, 0, :, :] +
                            0.5870 * rgb_tensor[:, 1, :, :] +
                            0.1140 * rgb_tensor[:, 2, :, :]
                        )
                        
                        return grayscale_tensor.unsqueeze(1)  # Add a channel dimension to match (N, 1, H, W)

                    outputs = torch.stack(outputs, dim=0)
                    outputs = rgb_to_grayscale_batch(outputs)
                    uncertainty_map = torch.std(outputs, dim=0).squeeze()

                    # Normalize for saving
                    uncertainty_map -= uncertainty_map.min()
                    uncertainty_map /= uncertainty_map.max()

                    # Save the uncertainty map
                    plt.imshow(uncertainty_map.cpu(), cmap='magma')
                    plt.colorbar(label='Uncertainty')
                    plt.axis('off')
                    plt.tight_layout()
                    plt.savefig(os.path.join(uncertainty_path, f'uncertainty_{x_name[i]}.png'))
                    plt.close()


    # def sample_to_eval_combined_with_uncertainty(self, net, test_loader, sample_path):
    #     def rgb_to_grayscale_batch(rgb_tensor):
    #         """
    #         Convert a batch of RGB tensors to grayscale.
            
    #         Args:
    #             rgb_tensor (torch.Tensor): Tensor of shape (N, C, H, W), where C = 3 for RGB.
                
    #         Returns:
    #             torch.Tensor: Grayscale tensor of shape (N, 1, H, W).
    #         """
    #         if rgb_tensor.size(1) != 3:
    #             raise ValueError("Input tensor must have 3 channels in the second dimension (N, C, H, W).")

    #         # Apply the grayscale conversion formula along the channel dimension
    #         grayscale_tensor = (
    #             0.2989 * rgb_tensor[:, 0, :, :] +
    #             0.5870 * rgb_tensor[:, 1, :, :] +
    #             0.1140 * rgb_tensor[:, 2, :, :]
    #         )
            
    #         return grayscale_tensor.unsqueeze(1)  # Add a channel dimension to match (N, 1, H, W)

    #     condition_path = make_dir(os.path.join(sample_path, 'condition'))
    #     gt_path = make_dir(os.path.join(sample_path, 'ground_truth'))
    #     result_path = make_dir(os.path.join(sample_path, str(self.config.model.BB.params.sample_step)))
    #     titled_plot_path = make_dir(os.path.join(sample_path, 'titled_plots'))

    #     pbar = tqdm(test_loader, total=len(test_loader), smoothing=0.01)
    #     batch_size = self.config.data.test.batch_size
    #     to_normal = self.config.data.dataset_config.to_normal
    #     sample_num = self.config.testing.sample_num

    #     from PIL import ImageDraw, ImageFont
    #     import numpy as np

    #     def add_titles_and_concat(condition, gt, result, uncertainty):
    #         """Adds titles to images and concatenates them side by side."""
    #         def add_title(img, title):
    #             draw = ImageDraw.Draw(img)
    #             font = ImageFont.load_default()  # Default PIL font
    #             text_bbox = font.getbbox(title)  # Bounding box of the text
    #             text_height = text_bbox[3] - text_bbox[1]  # Calculate text height
    #             draw.rectangle([(0, 0), (img.width, text_height + 5)], fill=(0, 0, 0))
    #             draw.text((5, 0), title, fill=(255, 255, 255), font=font)
    #             return img

    #         condition = add_title(condition, "Condition")
    #         gt = add_title(gt, "Ground Truth")
    #         result = add_title(result, "Output")
    #         uncertainty = add_title(uncertainty, "Uncertainty Map")

    #         total_width = condition.width + gt.width + result.width + uncertainty.width
    #         max_height = max(condition.height, gt.height, result.height, uncertainty.height)

    #         combined = Image.new('RGB', (total_width, max_height))
    #         combined.paste(condition, (0, 0))
    #         combined.paste(gt, (condition.width, 0))
    #         combined.paste(result, (condition.width + gt.width, 0))
    #         combined.paste(uncertainty, (condition.width + gt.width + result.width, 0))

    #         return combined

    #     for test_batch in pbar:
    #         (x, x_name), (x_cond, x_cond_name) = test_batch
    #         x = x.to(self.config.training.device[0])
    #         x_cond = x_cond.to(self.config.training.device[0])

    #         for i in range(batch_size):
    #             outputs = []

    #             for j in range(sample_num):
    #                 sample = net.sample(x_cond[i].unsqueeze(0), clip_denoised=False)
    #                 result = sample[0]

    #                 # Save individual images
    #                 if j == 0:
    #                     save_single_image(x_cond[i], condition_path, f'{x_cond_name[i]}.png', to_normal=to_normal)
    #                     save_single_image(x[i], gt_path, f'{x_name[i]}.png', to_normal=to_normal)

    #                 result_path_i = make_dir(os.path.join(result_path, x_name[i]))
    #                 save_single_image(result, result_path_i, f'output_{j}.png', to_normal=to_normal)

    #                 outputs.append(result.detach())

    #             # Calculate uncertainty map after all samples for the image
    #             from matplotlib import pyplot as plt
    #             from matplotlib import cm
    #             from PIL import Image

    #             # Calculate uncertainty map after all samples for the image
    #             if len(outputs) > 1:
    #                 outputs = torch.stack(outputs, axis=0)
    #                 grayscale_outputs = rgb_to_grayscale_batch(outputs)  # N, 1, H, W
    #                 uncertainty_map = torch.std(grayscale_outputs, axis=0).squeeze()

    #                 # Normalize uncertainty map for visualization
    #                 uncertainty_map -= uncertainty_map.min()
    #                 uncertainty_map /= uncertainty_map.max()

    #                 # Render uncertainty map with colormap and colorbar
    #                 plt.figure(figsize=(6, 6))
    #                 plt.imshow(uncertainty_map.cpu(), cmap='magma')
    #                 plt.colorbar(label='Uncertainty', shrink=0.8)
    #                 plt.title('Uncertainty Map')
    #                 plt.axis('off')

    #                 # Save the plot with colorbar
    #                 titled_plot_path_i = make_dir(os.path.join(titled_plot_path, x_name[i]))
    #                 colorbar_path = os.path.join(titled_plot_path_i, f'uncertainty_colormap_{x_name[i]}.png')
    #                 plt.savefig(colorbar_path, bbox_inches='tight')
    #                 plt.close()

    #                 # Apply magma colormap without colorbar for PIL
    #                 colormap = cm.get_cmap('magma')
    #                 uncertainty_colored = colormap(uncertainty_map.cpu().numpy())[:, :, :3]  # Remove alpha channel
    #                 uncertainty_colored = (uncertainty_colored * 255).astype(np.uint8)  # Convert to uint8
    #                 uncertainty_pil = Image.fromarray(uncertainty_colored)

    #                 # Create composite titled plot
    #                 condition_img = (x_cond[i] * 0.5 + 0.5).clamp(0, 1).mul(255).permute(1, 2, 0).cpu().numpy().astype(np.uint8)
    #                 gt_img = (x[i] * 0.5 + 0.5).clamp(0, 1).mul(255).permute(1, 2, 0).cpu().numpy().astype(np.uint8)
    #                 result_img = (result * 0.5 + 0.5).clamp(0, 1).mul(255).permute(1, 2, 0).cpu().numpy().astype(np.uint8)

    #                 condition_pil = Image.fromarray(condition_img)
    #                 gt_pil = Image.fromarray(gt_img)
    #                 result_pil = Image.fromarray(result_img)

    #                 combined_img = add_titles_and_concat(condition_pil, gt_pil, result_pil, uncertainty_pil)

    #                 # Save composite image
    #                 # titled_plot_path_i = make_dir(os.path.join(titled_plot_path, x_name[i]))
    #                 combined_img.save(os.path.join(titled_plot_path_i, f'titled_plot_{x_name[i]}.png'))



