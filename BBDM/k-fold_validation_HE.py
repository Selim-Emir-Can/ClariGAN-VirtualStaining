...
def CPU_singleGPU_launcher(config, train_set, val_set, test_set, save_name, label_by_name=None, class_weights=None):
    set_random_seed(config.args.seed)
...
def main(train_set, val_set, test_set, save_name, label_by_name=None, class_weights=None):
    ...
    if gpu_ids == "-1": # Use CPU
        ...
        if len(gpu_list) > 1:
            ...
            CPU_singleGPU_launcher(nconfig, train_set, val_set, test_set, save_name, label_by_name, class_weights)
    return nconfig
...
def stratified_kfold_85_5_10(train_dir="train", k=10, seed=42):
    ...
    r1_files = [f for f in os.listdir(input_dir) if os.path.isfile(os.path.join(input_dir, f))]

    data = []
    # pattern = re.compile(r'^(Skin[A-Z]\d|[A-Z]\d)_(i{1,3})_(BF|Stained)_r\d+_c\d+\.tif$', re.IGNORECASE)
    pattern = re.compile( 
        r"^("
        r"skinA3|skinA2|skinA1|"
        r"skinB3|skinB2|skinB1|"
        r"BCCMDFH|NECKsis|"
        r"3sisA|3BW|2sis|2BW|2BN|1BB|1BM|1BN|1BS|B2|B1"
        r")_(i{1,3})__\d+_\d+\.tif$",
        re.IGNORECASE
    )
    """
    pattern = re.compile(
        r"^(?:"
        r"skinA3|skinA2|skinA1|skinB3|skinB2|skinB1|"   # ‘skin…’ first so they don’t get
        r"BCCMDFH|NECKsis|"                             # masked by shorter prefixes
        r"3sisA|3BW|2sis|2BW|2BN|1BB|1BM|1BN|1BS|B2|B1"
        r")_(i{1,3})_(BF|HEreg)_r\d+_c\d+\.tif$",
        re.IGNORECASE
    )
    """
    ...
    for filename in r1_files:
        ...
        r3_filename = filename.replace("BF", "HEreg", 1)
...
def get_IFBCL_from_split(split):
    labels = []
    # Match 'SkinB1', 'B1', 'SkinA2', etc., at the start of the filename
    # pattern = re.compile(r'^(Skin[A-Z]\d|[A-Z]\d)_(i{1,3})', re.IGNORECASE)
    pattern = re.compile(
...
if __name__ == "__main__":
    splits = stratified_kfold_85_5_10(r"C:\Users\ammic\Downloads\train800\train", k=10) # change the path to BBDM dataset train directory
    ...
    sys.argv = [
        ...
        "--config", r"C:\Users\ammic\Desktop\ClariGAN-DL\BBDM\configs\BrianHE.yaml", # change it to your config file path
        "--train",
        "--sample_at_start",
        "--save_top",
        "--gpu_ids", "0"]
    ...
    for fold in splits:
        ...
        train_labels = get_IFBCL_from_split(train_set)
        counts = Counter(train_labels)
        beta = 0.999
        class_weights = {c: (1.0 - beta) / (1.0 - beta**cnt) for c, cnt in counts.items()}

        # Map filename → label so batches can look up weights
        label_by_name = {}  # fill with your r1_path basename → label
        for (r1_path, _), lbl in zip(train_set, train_labels):
            label_by_name[os.path.basename(r1_path).split("_")[0]] = lbl
        ...
        nconfig = main(train_set, val_set, test_set, save_name, label_by_name, class_weights)
        
        # === Evaluation ===
        nconfig.data.test.batch_size = 1
        nconfig.data.dataset_config.dataset_path = r"C:\Users\ammic\Downloads\train800\train" # change the path to BBDM dataset train directory
        ...
        sample_path = os.path.join(r"C:\Users\ammic\Desktop\ClariGAN-DL\BrianHE_k-fold_results", save_name) # WHERE YOU HAVE TEST RESULTS FOR EACH FOLD of the k-fold cross validation
